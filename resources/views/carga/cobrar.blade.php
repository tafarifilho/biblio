...
@section('content')
	...
	{!! Form::open(array('route'=>'carga.cobrar.post', 'class' => 'form-horizontal')) !!}

	<h1>Total de cargas vencidas: {{ count($cargas) }}</h1><br>
	...
	<table id="eventsTable"
			...
			<thead>
				<tr>
					...
					<th data-field="informacoes" >informações</th>
					<th data-field="controle" >controle</th>
			...
			<tbody>
				@foreach ($cargas as $carga)
					...
					@endif
						...
						<td>
							@if ($carga->autoridade->telefone)
								@foreach ($carga->autoridade->telefone as $telefone)
								...
								@endforeach
							@else
								@if ($carga->predio->tronco)
									Tronco: {{ $carga->predio->tronco}}
								@endif
							@endif
						</td>
						<td>
							@if ($carga->tiposolicitacao->tipo)
								<strong>{{ $carga->tiposolicitacao->tipo }}</strong>
							@endif
							@if ($carga->solicitante)
								(Solicitante: {{ $carga->solicitante }})
							@endif
							@if ($carga->retirante)
								(Retirante: {{ $carga->retirante }})
							@endif
							@if ($carga->observacao)
								(Obs: {{ $carga->observacao }})
							@endif
						</td>
						<td>
							@foreach ($carga->controles as $controle)
								<strong>{{ $controle->created_at->format('d\/m\/Y') }}</strong>
